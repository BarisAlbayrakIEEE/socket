...
namespace BA_Socket {
    ...
    class Handler_Accept : public IHandler {
    public:
        std::vector<fd_Action> apply(int fd) const {
            ...
            return { { Enum_fd_Action_Types::Register_Read_Write, fd_client } };
        };
    };

    class Handler_Read : public IHandler {
    public:
        std::vector<fd_Action> apply(int fd) const {
            char read[1024];
            int bytes_received = ::recv(fd, read, 1024, 0);
            PRINTF1("[Server]: Receiving data from client...\n");
            if (bytes_received < 1) SOCKET_ERROR__RECV();
    ...
    class Handler_Write : public IHandler {
        std::string buf;
    public:
        std::vector<fd_Action> apply(int fd) const {
    ...
    class Handler_Read_Write : public IHandler {
    public:
        std::vector<fd_Action> apply(int fd) const {
            char read[1024];
            int bytes_received = ::recv(fd, read, 1024, 0);
